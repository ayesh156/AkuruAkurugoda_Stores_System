...
public class GrnHistory extends javax.swing.JPanel {
    ...
    private void searchGRN() {
        ...
        String suplierMobile = jTextField26.getText();
        String date = jFormattedTextField10.getText().trim();

        try {

            ResultSet rs = MySQL.execute("SELECT * FROM grn INNER JOIN employee ON employee.email=grn.employee_email  INNER JOIN supplier "
                    + "ON grn.supplier_mobile=supplier.mobile WHERE grn.id LIKE '%" + grnID + "%' AND grn.supplier_mobile LIKE '%" + suplierMobile + "%' "
                    + " AND grn.date_time LIKE '%" + date + "%'  AND employee.email LIKE '%" + employeeEmail + "%' AND ( type='cashier' OR type='superAdmin' ) ");
            ...
            while (rs.next()) {
                ...
                v.add(rs.getString("supplier_mobile"));
                ...
                String supplierId = rs.getString("supplier_mobile");

                if (!processedSuppliers.contains(supplierId)) {
                    double payment = Double.parseDouble(rs.getString("supplier.balance_amount"));
                    totalPayments += payment;

                    // Mark the supplier as processed
            ...
            jLabel2.setText(String.valueOf(totalPayments));
    ...
    private void loadGRN() {
        try {
            ...
            while (resultset.next()) {
                ...
                String supplierId = resultset.getString("supplier_mobile");

                if (!processedSuppliers.contains(supplierId)) {
                    double payment = Double.parseDouble(resultset.getString("supplier.balance_amount"));
                    totalPayments += payment;
    ...
    private void initComponents() {
        ...
        jTextField26.setPreferredSize(new java.awt.Dimension(200, 22));
        jTextField26.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                jTextField26KeyReleased(evt);
            }
        });
        jPanel3.add(jTextField26, java.awt.BorderLayout.CENTER);

        jLabel74.setBackground(new java.awt.Color(153, 204, 255));
        jLabel74.setFont(new java.awt.Font("Segoe UI Semibold", 0, 18)); // NOI18N
        jLabel74.setText("<html> <p>Supplier mobile</p> <html>");
        jLabel74.setPreferredSize(new java.awt.Dimension(150, 40));
        jPanel3.add(jLabel74, java.awt.BorderLayout.WEST);
        ...
        jLabel75.setFont(new java.awt.Font("Segoe UI Semibold", 0, 18)); // NOI18N
        jLabel75.setForeground(new java.awt.Color(0, 153, 255));
        jLabel75.setText("Pending payments");
        jLabel75.setPreferredSize(new java.awt.Dimension(200, 25));
        jPanel1.add(jLabel75, java.awt.BorderLayout.WEST);
        ...
        jTable9.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "GRN ID", "Supplier mobile", "Employee", "Date amd Time", "Paid amount", "GRN Total"
            }
        ) {
        ...
        if (jTable9.getColumnModel().getColumnCount() > 0) {
            ...
            jTable9.getColumnModel().getColumn(2).setPreferredWidth(40);
            jTable9.getColumnModel().getColumn(4).setPreferredWidth(0);
    ...
    private void jTable9MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable9MouseClicked
        ...
        try {
            ResultSet resultset = MySQL.execute("SELECT * FROM `grn_history_item` "
                    ...
                    + "INNER JOIN `supplier` ON `grn`.`supplier_mobile`=`supplier`.`mobile` WHERE `grn_id`='" + grnID + "'");
            ...
            while (resultset.next()) {
                ...
                grnTotal += itemTotal;
                totalItems++;
                payments = Double.parseDouble(resultset.getString("supplier.balance_amount"));
            }

            jLabel2.setText(String.valueOf(payments));
    ...
    private void jButton39ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton39ActionPerformed
        ...
        String comapny = jTextField25.getText();
        String suplierMobile = jTextField26.getText();
        String date = jTextField1.getText().trim();

        try {

            ResultSet rs = MySQL.execute("SELECT * FROM `grn` INNER JOIN `supplier` ON `grn`.`supplier_mobile`=`supplier`.`mobile` INNER JOIN `company` ON "
                    + "`supplier`.`company_id`=`company`.`id`WHERE `grn`.`id` LIKE '" + grnID + "%' AND `grn`.`supplier_mobile` LIKE '" + suplierMobile + "%' "
                    + " AND `grn`.`date_time` LIKE '" + date + "%'  AND `company`.`name` LIKE '" + comapny + "%' AND `employee_email` != 'admin@gmail.com'   ;");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        try {
            ...
            for (int i = 0; i < jTable9.getRowCount(); i++) {
                netTotal += Double.parseDouble(String.valueOf(jTable9.getValueAt(i, 5)));
            }
...
//                    .toString();
//
//            String url = userDirectory + "\\src\\reports\\new_grn_report.jasper";
//            String suburl = userDirectory + "\\src\\reports\\grn_items.jasper";
//
...
//accesing the json file
            ...
            parameters.put("Parameter1", String.valueOf(netTotal));
            parameters.put("Parameter2", date);
            parameters.put("Parameter3", jLabel2.getText());

            JRTableModelDataSource datasource = new JRTableModelDataSource(jTable9.getModel());
            JasperFillManager.fillReport(url, parameters, connection);
